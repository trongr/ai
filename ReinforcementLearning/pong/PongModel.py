...
from RLPolicyGradientSolver import RLPolicyGradientSolver
...
class PongPreprocessor(object):
    ...
    def preprocess(self, img):
        ...
        img[img == 109] = 0
        img[img != 0] = 1
        curr = mx.nd.array(img.astype(np.float).ravel())
        curr = mx.ndarray.expand_dims(curr, axis=0)
        curr = mx.nd.array(curr)
        # Subtract the last preprocessed image.
        diff = (
            curr - self.prev
            if self.prev is not None
            else mx.nd.zeros((1, curr.shape[1]))
        )
        self.prev = curr
        return diff
...
class PolicyNetwork(ModelBase):
    def __init__(
        self, preprocessor, input_size=80 * 80, hidden_size=200, gamma=0.99
    ...
    def forward(self, X):
        ...
        w1 = mx.nd.array(self.params["w1"])
        a = mx.ndarray.dot(w1, X.T)
        h = mx.ndarray.maximum(0, a)
        w2 = mx.nd.array(self.params["w2"])
        logits = mx.ndarray.dot(h.T, w2.T)
        p = 1.0 / (1.0 + mx.ndarray.exp(-logits))
        return p

    def choose_action(self, p):
        """Return an action `a` and corresponding label `y` using the probability float `p`."""
        a = 2 if np.random.uniform() < p else 3
        y = 1 if a == 2 else 0
        return a, y

    def loss(self, ps, ys, rs):
        # Prevent log of zero.
        ps = mx.ndarray.maximum(1.0e-5, mx.ndarray.minimum(1.0 - 1e-5, ps))
        step_losses = ys * mx.ndarray.log(ps) + (1.0 - ys) * mx.ndarray.log(1.0 - ps)
        return -mx.ndarray.sum(step_losses * rs)

    def discount_rewards(self, rs):
        drs = mx.ndarray.zeros_like(mx.nd.array(rs))
        drs = mx.nd.array(drs).asnumpy()
        s = 0
