...
class RLPolicyGradientSolver(Solver):
    ...
    def run_episode(self):
        ...
        self.running_reward = (
            ...
            else (0.99 * self.running_reward + 0.01 * self.episode_reward)
        )
        xs = np.vstack(xs)
        ys = np.vstack(ys)
    ...
    def train(self):
        ...
        for episode_number in range(1, self.num_episodes):
            ...
            grad_and_loss_func = grad_and_loss(
            ...
            if self.verbose or episode_number % self.print_every == 0:
                print(
                    "Episode %d complete (%.2fs), loss: %s, reward: %s, running reward: %s"
                    % (
                        episode_number,
                        episode_time,
                        loss,
            ...
            if episode_number % self.save_every == 0:
                ...
                with open(file_name, "wb") as f:
                    pickle.dump(
                        {k: v.asnumpy() for k, v in self.model.params.items()}, f
                    )
                if self.verbose:
