...
class A2CPolicy(object):
    ...
    def __init__(self, sess, ob_space, action_space, nbatch, nsteps, reuse=False):
        ...
        # type we will use to distribute action in our stochastic policy, in our
        # case DiagGaussianPdType aka Diagonal Gaussian, 3D normal distribution
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            # This build a fc connected layer that returns a probability
            # distribution over actions (self.pd) and our pi logits (self.pi).
        ...
        # Take an action in the action distribution (remember we are in a
        # situation of stochastic policy so we don't always take the action with
        # the highest probability for instance if we have 2 actions 0.7 and 0.3
        # we have 30% chance to take the second)
        a0 = self.pd.sample()
