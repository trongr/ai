...
def discriminator(x):
    with tf.variable_scope("discriminator"):
        ...
        c1 = tf.layers.conv2d(inputs=input_layer, filters=32, kernel_size=5, strides=1, padding='same', activation=leaky_relu)
        c2 = tf.layers.conv2d(inputs=c1, filters=64, kernel_size=5, strides=1, padding='same', activation=leaky_relu)
...
def generator(z, keep_prob):
    with tf.variable_scope("generator"):
        fc1 = tf.contrib.layers.fully_connected(z, num_outputs=1024, activation_fn=tf.nn.relu)
        bn1 = tf.layers.batch_normalization(fc1, axis=-1, momentum=0.99, epsilon=0.001, center=True, scale=True, training=True)

        in_channels_1 = 128
        fc2 = tf.contrib.layers.fully_connected(bn1, num_outputs=7 * 7 * in_channels_1, activation_fn=tf.nn.relu)
        bn2 = tf.layers.batch_normalization(fc2, axis=-1, momentum=0.99, epsilon=0.001, center=True, scale=True, training=True)
        rs1 = tf.reshape(bn2, [-1, 7, 7, in_channels_1])

        in_channels_2 = 64
        ct1_filter = tf.get_variable("ct1_filter", shape=(4, 4, in_channels_2, in_channels_1), dtype=tf.float32, initializer=tf.truncated_normal_initializer(), trainable=True)
        ct1 = tf.nn.relu(tf.nn.conv2d_transpose(rs1, filter=ct1_filter, output_shape=(batch_size, 14, 14, in_channels_2), strides=(1, 2, 2, 1), padding='SAME', data_format='NHWC'))
        bn3 = tf.layers.batch_normalization(ct1, axis=-1, momentum=0.99, epsilon=0.001, center=True, scale=True, training=True)

        ct2_filter = tf.get_variable("ct2_filter", shape=(4, 4, 1, in_channels_2), dtype=tf.float32, initializer=tf.truncated_normal_initializer(), trainable=True)
        ct2 = tf.nn.relu(tf.nn.conv2d_transpose(bn3, filter=ct2_filter, output_shape=(batch_size, 28, 28, 1), strides=(1, 2, 2, 1), padding='SAME', data_format='NHWC'))

        avg = tf.reduce_mean(ct2, axis=3, keep_dims=True)
        rs2 = tf.reshape(avg, [-1, x_dim])
        img = tf.contrib.layers.fully_connected(rs2, num_outputs=x_dim, activation_fn=tf.tanh)
        return img
