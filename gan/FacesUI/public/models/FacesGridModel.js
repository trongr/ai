const FacesGridModel = (() => {
  ...
  FacesGridModel.saveEncodings = (
    ...
    assert(
      nencodings.length == NUM_CELLS_ROWS * NUM_CELLS_COLS,
      `${tag}: Random faces encoding length ${
        nencodings.length
      } doesn't match expected number of rows ${NUM_CELLS_ROWS} and columns ${NUM_CELLS_COLS}`,
    )
    let encodings = []
